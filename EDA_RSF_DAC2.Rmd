...
Summer 2022 data. 
```{r}
#### Importing summer locations, making tibble for RSF model, viewing results #### 
...
rsfData_summer2022 <- Summer_2022_locs %>% 
  ...
  mutate(indData = map2(tracks, kde, function(x,y){
      ...
      random_points(y, n = nrow(x) * 20, level=.95, presence = x) %>%
...
competing_summer_2022 <- model.sel(pop.dre_summer_2022[1:9])#, subset = "delta" < 2)
