...
np_bu_pred_draws_sum <- np_bu_pred_draws_sum %>%
  mutate(
    NP_BU_original = (NP_BU * 97.47) + 80.74  # Transform scaled to original
  )
...
NP_BU_og <- np_bu_pred_draws_sum %>%
  ...
  theme_bw() +
  scale_x_continuous(breaks = c(0, 400, 800)) +
  scale_y_continuous(breaks = c(0.05, 0.1, 0.15), limits = c(0, 0.15), labels = NULL) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    axis.line = element_line("black"),
...
final_faceted_plot + 
  plot_annotation(
    x = "Distance (original scale)",  # Add x-axis title
    y = "Probability of selection"   # Add y-axis title
  )
