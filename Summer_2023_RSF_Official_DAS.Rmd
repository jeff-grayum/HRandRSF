...
habitat <- raster::shapefile("/Volumes/Samsung_T5/ThesisStudySite/LCOV_UPDATE/2023_FL_RC/2023_shapefile.shp")
...
Summer_2023_locs <- rio::import("/Volumes/Samsung_T5/R_projects/_THESIS/Chapter1/Summer2023Locs.xlsx", setclass = "tibble") %>%
  clean_names() %>%
  filter(status == "A") %>%
...
How many locations were in each land cover type?
```{r}
#### Num/percent of locs in each land cover type ####

#importing map
habitat23 <- raster::shapefile("/Volumes/Samsung_T5/ThesisStudySite/LCOV_UPDATE/2023/UPDATED_2023_LCOV_MGMT.shp")

# Converting summer_2023_locs to SpatialPointsDataFrame
coordinates(Summer_2023_locs) <- ~easting + northing 

#Matching coord system with habitat layer
proj4string(Summer_2023_locs) <- proj4string(habitat23)

# Overlay the bobwhite locations onto the habitat map
bobwhite_lcov23 <- over(Summer_2023_locs, habitat23)

# Add lcov type to Summer 2023 locs
Summer_2023_locs$LCOV_MGMT <- bobwhite_lcov23$LCOV_MGMT

# Now summer_2023_locs contains the landcover type each bobwhite was in

# Plotting locations on the habitat map to make sure stuff makes sense
plot(habitat)
points(Summer_2023_locs, col = 'red', pch = 20) # change color and point character as needed

Summer_2023_locs <- Summer_2023_locs %>%
  as.tibble()

OVER_25_LOCS <- Summer_2023_locs %>%
  count(band_numb, sort = TRUE) %>%
  dplyr::filter(n >= 25) %>%
  dplyr::pull(band_numb)


#Filtering for birds with 25> locs
Summer_2023_locs25 <- Summer_2023_locs %>%
  dplyr::filter(band_numb %in% OVER_25_LOCS) 

Summer_2023_locs25LCOV <- Summer_2023_locs %>%
  dplyr::select(LCOV_MGMT)

#Combining with summer 2022 lcov
sum22_23lcov <- rbind(Summer_2022_locs25LCOV, Summer_2023_locs25LCOV)

#Making tibble of number of lcov 
LCOV_SUM <- sum22_23lcov %>%
  count(LCOV_MGMT, sort = TRUE)

#Making tibble of number of lcov 
  <- Summer_2023_locs25LCOV %>%
  count(LCOV_MGMT, sort = TRUE)

LCOV_SUM$percentLOCS <- ((LCOV_SUM$n / 4036) * 100)

#Counting 2023 lcov acres
#Let's count the land cover types.
lcov_sums23 <- habitat23 %>%
  as.data.frame() %>%
  group_by(LCOV_MGMT) %>%
  summarize(sum_acres23 = sum(ACRES))

lcov_sums22_23 <- cbind(lcov_sums22, lcov_sums23 %>% select(sum_acres23))



totalacres23 <- habitat23 %>%
  as.tibble () 

totalacres23 <- sum(totalacres23$ACRES)


totalacres <- sum(totalacres22 + totalacres23)

lcov_percent <- lcov_sums22_23 %>%
  group_by(LCOV_MGMT) %>%
  summarize(percentLCOV = ((sum_acres22 + sum_acres23) / totalacres) * 100)

percLocsLCOV <- lcov_percent %>%
  inner_join(LCOV_SUM %>% select(LCOV_MGMT, percentLOCS), by = "LCOV_MGMT") %>%
  arrange(-percentLCOV)
...
rsfData22_23_UCS <- rbind(rsfData_summer22_UCS, rsfData_summer23_UCS)
```
...
saveRDS(pop.rsf_summer2022_23_bayes, "/Volumes/Samsung_T5/BOBWHITE_DATA/Clean/Other/UPDATED_model_RSFsummer2022_23_FL_2.rds")
```

Summer 22/23 females only
```{r}
#### Summer 2022 AND 2023;#### 
...
#Saving model results
saveRDS(pop.rsf_summer2022_23_bayes_female, "/Volumes/Samsung_T5/BOBWHITE_DATA/Clean/Other/UPDATED_model_RSFsummer2022_23_female.rds")
```

Summer 22/23 males only
```{r}
#### Summer 2022 AND 2023;#### 
...
#Saving model results
saveRDS(pop.rsf_summer2022_23_bayes_male, "/Volumes/Samsung_T5/BOBWHITE_DATA/Clean/Other/UPDATED_model_RSFsummer2022_23_male.rds")
```

Visualizing data for all birds 22/23
```{r} 

#### Visualizing summer 22/23 rsf data ####
#Printing summary of results.

pop.rsf_summer2022_23_bayes <- readRDS("/Volumes/Samsung_T5/BOBWHITE_DATA/Clean/Other/UPDATED_model_RSFsummer2022_23_FL_2.rds")
...
posterior_draws_long %>%
  ...
  facet_wrap(~factor(landcover, levels = c("FL", "NP_BU", "SS_UB", "HP_UB", "WFP_FF", "PP_UB", "AG",  "PP_BU", "UM", "SS_BU", "NP_UB", "HW", "HP_BU")), ncol = 4) +
  theme_minimal() +
...
# Updated plot with rotated x-axis labels and a horizontal line at y = 1
coefficients_data_no_intercept_2022_2023 %>%
   mutate(
     variable = factor(variable, levels = names(new_names)), # Ensure factor levels match original names
     variable = new_names[variable], # Replace with new names
     odds_ratio = round(odds_ratio, 2) # Round odds ratio to 2 decimal places
   ) %>% 
  ggplot(aes(y = reorder(variable, -estimate), x = odds_ratio)) +
  geom_point() +
  geom_vline(xintercept = 1, linetype = "dashed", color = "gray") + # Add dashed vertical line at 1
  theme_bw() +
  labs(y = "Land cover", x = "Odds Ratio") +
  theme(legend.position = "none",
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.line = element_line(color = "black"),
...
Visualizing data for female birds 22/23
```{r} 

#### Visualizing summer 22/23 rsf data ####
#Printing summary of results.

pop.rsf_summer2022_23_bayes_f <- readRDS("/Volumes/Samsung_T5/BOBWHITE_DATA/Clean/Other/UPDATED_model_RSFsummer2022_23_female.rds")
...
pop.rsf_summer2022_23_bayes_m <- readRDS( "/Volumes/Samsung_T5/BOBWHITE_DATA/Clean/Other/UPDATED_model_RSFsummer2022_23_male.rds")
...
coefficients_data_no_intercept_allBU_UB <- read_xlsx("/Users/jeffgrayum/Downloads/UPDATED_all_bu_ubFixed.xlsx")

#Renaming 
coefficients_data_no_intercept_allBU_UB <- coefficients_data_no_intercept_allBU_UB %>%
  mutate(variable = case_when(
    variable == "bu" ~ "<1Year",
    variable == "ub" ~ "1-2Years",
    variable == "n_b" ~ "NB"
  ))
...
coefficients_data_no_intercept_allBU_UB %>%
  ggplot(aes(x = reorder('Time since fire', -β), y = β)) +
  geom_point() + 
  geom_errorbar(aes(ymin = l_95_percent_ci, ymax = u_95_percent_ci), width = 0.2) + 
  geom_hline(yintercept = 0, linetype = "dashed", color = "black", linewidth = 0.5) +  
  coord_flip() +  
...
coefficients_data_no_intercept_allBU_UB %>%
  ggplot(aes(x = reorder(`Time since fire`, -`β`), y = `β`)) +
  geom_point() + 
  geom_errorbar(aes(ymin = `Lower 95% Cr.I.`, ymax = `Upper 95% Cr.I.`), width = 0.2) + 
  geom_hline(yintercept = 0, linetype = "dashed", color = "black", linewidth = 0.5) +  
  coord_flip() +  
  labs(x = "Time since fire",
       y = "Estimated posterior beta coefficient") +
  ...
  theme(panel.grid.major = element_blank(),
        ...
        legend.text = element_text(colour="black", size=12, family="Times New Roman"),
        axis.text.y = element_text(colour="black", size=12, family="Times New Roman"),
        text = element_text(family = "Times New Roman"))  # Set global text elements to Times New Roman
...
# Pivoting long, so we can facet wrap by land cover type
posterior_draws_long <- posterior_draws %>%
  pivot_longer(cols = `<1 Year`:`1-2 Years`, names_to = "treatment", values_to = "value")
...
posterior_draws_long %>%
  dplyr::filter(treatment != "NB") %>%
  ggplot(aes(x = value, fill = treatment)) +
  geom_density(alpha = 0.7) +
  facet_wrap(~factor(treatment, levels = c("<1 Year", "1-2 Years"))) +
...
fl_pred_draws_sum%>% 
  ...
  geom_line() +
  geom_ribbon(aes(ymin = low_ci, ymax = upper_ci), fill = "black", alpha = 0.4) +
  labs(x = "Distance to FL (centered and scaled)", 
...
# Reverse centering and scaling for FL
fl_pred_draws_sum <- fl_pred_draws_sum %>%
  mutate(
    FL_original = (FL * 103.83) + 79.73  
  )

# Plot using the original x-axis values for FL
FL_og <- fl_pred_draws_sum %>%
  ggplot(aes(FL_original, mean_fit)) +
  geom_line() +
  geom_ribbon(aes(ymin = low_ci, ymax = upper_ci), fill = "black", alpha = 0.6) +
  labs(title = "Feed line", x = NULL, y = NULL) +
  theme_bw() +
  scale_x_continuous(breaks = c(0, 400, 800)) +
  scale_y_continuous(breaks = c(0.05, 0.1, 0.15), limits = c(0.025, 0.15)) +
 theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    axis.line = element_line("black"),
    axis.ticks = element_line("black"),
    plot.title = element_text(colour = "black", size = 10, family = "Times New Roman"),
    ...
    axis.title.x = element_text(colour = "black", size = 10, family = "Times New Roman"),
...
# Reverse centering and scaling np_bu
np_bu_pred_draws_sum <- np_bu_pred_draws_sum %>%
  mutate(
    NP_BU_original = (NP_BU * 97.75) + 81.44  # Transform scaled to original
  )

# Plot using the original x-axis values
NP_BU_og <- np_bu_pred_draws_sum %>%
  ggplot(aes(NP_BU_original, mean_fit)) +
  geom_line() +
  geom_ribbon(aes(ymin = low_ci, ymax = upper_ci), fill = "#006400", alpha = 0.4) +
  labs(title = "Natural pine (B)", x = NULL, y = NULL) +
  theme_bw() +
  scale_x_continuous(breaks = c(0, 400, 800)) +
   scale_y_continuous(breaks = c(0.05, 0.1, 0.15), limits = c(0.025, 0.15), labels = NULL) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    axis.line = element_line("black"),
...
ss_ub_pred_draws_sum %>% 
  ...
  geom_line() +
  geom_ribbon(aes(ymin = low_ci, ymax = upper_ci), fill = "gray50", alpha = 0.5) +
...
# Reverse centering and scaling for SS_UB
ss_ub_pred_draws_sum <- ss_ub_pred_draws_sum %>%
  mutate(
    SS_UB_original = (SS_UB * 116.87) + 144.02  
  )

# Plot using the original x-axis values for SS_UB
SS_UB_og <- ss_ub_pred_draws_sum %>% 
  ggplot(aes(SS_UB_original, mean_fit)) +
  geom_line() +
  geom_ribbon(aes(ymin = low_ci, ymax = upper_ci), fill = "gray50", alpha = 0.5) +
  labs(title = "Shrub/scrub (U)", x = NULL, y = NULL) +
  theme_bw() +
  scale_x_continuous(breaks = c(0, 400, 800)) +
   scale_y_continuous(breaks = c(0.05, 0.1, 0.15), limits = c(0.025, 0.15), labels = NULL) +
  theme(
...
hp_ub_pred_draws_sum%>% 
  ...
  labs(x = "Distance to HP_UB (centered and scaled)", 
       y = "Probability of selection") +
  theme_bw() +
  theme(panel.grid.major=element_blank(),
        panel.grid.minor=element_blank(),
...
# Reverse centering and scaling for HP_UB
hp_ub_pred_draws_sum <- hp_ub_pred_draws_sum %>%
  mutate(
    HP_UB_original = (HP_UB * 154.16) + 193.21 
  )

# Plot using the original x-axis values for HP_UB
HP_UB_og <- hp_ub_pred_draws_sum %>%
  ggplot(aes(HP_UB_original, mean_fit)) +
  geom_line() +
  geom_ribbon(aes(ymin = low_ci, ymax = upper_ci), fill = "brown", alpha = 0.4) +
  labs(title = "Hardwood/pine (U)", x = NULL, y = NULL) +
  theme_bw() +
  scale_x_continuous(breaks = c(0, 400, 800)) +
   scale_y_continuous(breaks = c(0.05, 0.1, 0.15), limits = c(0.025, 0.15), labels = NULL) +
  theme(
...
# Reverse centering and scaling for NP_UB
np_ub_pred_draws_sum <- np_ub_pred_draws_sum %>%
  mutate(
    NP_UB_original = (NP_UB * 76.89) + 72.48
  )

# Plot using the original x-axis values for NP_UB
NP_UB_og <- np_ub_pred_draws_sum %>%
  ggplot(aes(NP_UB_original, mean_fit)) +
  geom_line() +
  geom_ribbon(aes(ymin = low_ci, ymax = upper_ci), fill = "#006400", alpha = 0.4) +
  labs(title = "Natural pine (U)", x = NULL, y = NULL) +
  theme_bw() +
   scale_x_continuous(breaks = c(0, 250, 500), limits = c(-0.9, 500)) +
    scale_y_continuous(breaks = c(0.05, 0.1, 0.15, 0.2), limits = c(0.05, 0.2)) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    axis.line = element_line("black"),
...
ss_bu_df <- data.frame(
  ...
  SS_UB = rep(mean(rsf_modified_2022_23$SS_UB), length(ss_bu_values)),
  SS_BU = ss_bu_values,
...
# Reverse centering and scaling for SS_BU
ss_bu_pred_draws_sum <- ss_bu_pred_draws_sum %>%
  mutate(
    SS_BU_original = (SS_BU * 170.3421) + 204.9688  # Transform scaled to original
  )

# Plot using the original x-axis values for SS_BU
SS_BU_og <- ss_bu_pred_draws_sum %>% 
  filter(SS_BU_original >= 0) %>%
  ggplot(aes(SS_BU_original, mean_fit)) +
  geom_line() +
  geom_ribbon(aes(ymin = low_ci, ymax = upper_ci), fill = "gray50", alpha = 0.5) +
  labs(title = "Shrub/scrub (B)", x = NULL, y = NULL) +
  theme_bw() +
   scale_x_continuous(breaks = c(0, 500, 1000)) +
    scale_y_continuous(breaks = c(0.05, 0.1, 0.15, 0.2), limits = c(0.05, 0.2), labels = NULL) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    axis.line = element_line("black"),
...
# Reverse centering and scaling for HP_BU
hp_bu_pred_draws_sum <- hp_bu_pred_draws_sum %>%
  mutate(
    HP_BU_original = (HP_BU * 205.82) + 234.62 
  )

# Plot using the original x-axis values for HP_BU
HP_BU_og <- hp_bu_pred_draws_sum %>% 
  filter(HP_BU_original >= 0) %>%
  ggplot(aes(HP_BU_original, mean_fit)) +
  geom_line() +
  geom_ribbon(aes(ymin = low_ci, ymax = upper_ci), fill = "brown", alpha = 0.5) +
  labs(title = "Hardwood/pine (B)", x = NULL, y = NULL) +
  theme_bw() +
...
final_faceted_plot <- (FL_og + NP_BU_og + SS_UB_og + HP_UB_og + 
                       NP_UB_og + SS_BU_og + HP_BU_og + 
...
#### Making a facet-wrapped plot pred plot ####
all_covs_pred %>%
  ggplot(aes(value, mean_fit)) +
  geom_line() +
  geom_ribbon(aes(ymin = low_ci, ymax = upper_ci, fill = Covariate), alpha = 0.6) +
  scale_color_manual(values = c("FL" = "black", "SS_UB" = "gray50", "SS_BU" = "gray50", "NP_BU" = "#006400", "NP_UB" = "#006400", "HP_BU" = "brown", "HP_UB" = "brown")) +
  scale_fill_manual(values = c("FL" = "black", "SS_UB" = "gray50", "SS_BU" = "gray50", "NP_BU" = "#006400", "NP_UB" = "#006400", "HP_BU" = "brown", "HP_UB" = "brown")) +
  labs(x = "Distance to covariate (centered and scaled)", 
       y = "Probability of selection") +
...
# Update the covariate labels in the dataset
all_covs_pred <- all_covs_pred %>%
...
all_covs_pred %>%
  ...
  geom_line() +
  geom_ribbon(aes(ymin = low_ci, ymax = upper_ci, fill = Covariate), alpha = 0.6) +
  scale_color_manual(values = c("Feed line" = "black",
                                "Shrub/scrub (U)" = "gray50",
                                "Shrub/scrub (B)" = "gray50",
                                "Natural pine (B)" = "#006400",
                                "Natural pine (U)" = "#006400",
                                "Hardwood/pine (B)" = "brown",
                                "Hardwood/pine (U)" = "brown")) +
  scale_fill_manual(values = c("Feed line" = "black",
                                "Shrub/scrub (U)" = "gray50",
                                "Shrub/scrub (B)" = "gray50",
                                "Natural pine (B)" = "#006400",
  ...
  theme(legend.position = "none",
        ...
        legend.title = element_text(color = "black", size = 12, family = "Times New Roman"),
        legend.text = element_text(color = "black", size = 12, family = "Times New Roman"),
        strip.text = element_text(color = "black", size = 12, family = "Times New Roman")) # Adjust facet text
...
# Renaming covariates directly
all_covs_pred %>%
  mutate(Covariate = factor(Covariate, levels = c("NP_BU", "AG", "SS_UB", "NP_UB", "HW", "HP_BU"))) %>%
  mutate(Covariate = recode(Covariate,
                            "NP_BU" = "Natural pine (B)",
                            "AG" = "Agriculture",
                            "SS_UB" = "Shrub/scrub (U)",
  ...
  geom_line() +
  geom_ribbon(aes(ymin = low_ci, ymax = upper_ci, fill = Covariate), alpha = 0.6) +
  scale_color_manual(values = c("Hardwood" = "#422305", 
                                "Shrub/scrub (U)" = "gray50", 
                                "Agriculture" = "#F0E68C", 
                                "Natural pine (B)" = "#006400", 
                                "Natural pine (U)" = "#006400", 
                                "Hardwood/pine (B)" = "brown")) +
  scale_fill_manual(values = c("Hardwood" = "#422305", 
                               "Shrub/scrub (U)" = "gray50", 
                               "Agriculture" = "#F0E68C", 
                               "Natural pine (B)" = "#006400", 
                               "Natural pine (U)" = "#006400", 
...
all_covs_pred %>%
  ...
  geom_line() +
  geom_ribbon(aes(ymin = low_ci, ymax = upper_ci, fill = Covariate), alpha = 0.6) +
  scale_color_manual(values = c("HW" = "#422305", "SS_UB" = "gray50", "AG" = "#F0E68C", "NP_BU" = "#006400", "NP_UB" = "#006400", "HP_BU" = "brown")) +
  scale_fill_manual(values = c("HW" = "#422305", "SS_UB" = "gray50", "AG" = "#F0E68C", "NP_BU" = "#006400", "NP_UB" = "#006400", "HP_BU" = "brown")) +
  labs(x = "Distance to covariate (centered and scaled)", 
       y = "Probability of selection") +
  facet_wrap(~ Covariate, scales = "free_x") +
  theme_bw() +
...
```{r}
#### Cross-validating our model with ppc_dense_overlay ####
...
posterior_samples <- posterior_predict(
  pop.rsf_summer2022_23_bayes, 
  draws = 100  # Use only 100 posterior draws
)


# Observed data
observed_data <- rsf_modified_2022_23$case

# Create density overlay plot
ppc_dens_overlay(observed_data, posterior_samples)
...
ppc_dens_overlay(observed_data, posterior_samples[1:100, ])
...
#Trying loo
pop.rsf_summer2022_23_bayes <- brm(
  case | trials(1) ~ AG + FL + UM + NP_UB + NP_BU + PP_UB + PP_BU +
    HW + HP_UB + HP_BU + SS_UB + SS_BU + WFP_FF + (1 | band_numb),
  data = rsf_modified_2022_23,
  family = "binomial",
  iter = 1000,
  chains = 4,
...
loo_result <- loo(pop.rsf_summer2022_23_bayes)
print(loo_result)
