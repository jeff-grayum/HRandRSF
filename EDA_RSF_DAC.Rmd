...
#Rename landcover types to model friendly terms. Lumping landcover types w similar understory vegetation.
...
habitat$HABITAT <- gsub("PINE/HARDWOOD", "NPF", habitat$HABITAT)
...
habitat$HABITAT <- gsub("HW/PINE FOREST", "HW", habitat$HABITAT)
habitat$HABITAT <- gsub("NATURAL PINE", "NPF", habitat$HABITAT)
habitat$HABITAT <- gsub("WILDLIFE FOOD PLOT", "AG", habitat$HABITAT)
habitat$HABITAT <- gsub("NP/PINE FOREST", "NPF", habitat$HABITAT)
...
Creating our stack of raster layers.
```{r}
...
for(i in 1:length(habitats)){   #We'll loop through each lcov type in "habitats".
  ...
  hab$field <- 1 
  # plot(hab) (unnecessary unless you want to plot each layer as the loop runs)
...
#Fitting glmm, 
pop.rsf_winter_21 <- glmer(case ~ AG + HW +  NPF + PP + SS + UM + WL + (1|covey_id),
...
Summer 2022. Repeating the process.
```{r}
...
pop.rsf_summer2022 <- glmer(case ~ AG + HW + NPF + PP + SS + UM + WL + (1|band_numb),
