...
#Without this mutation, below function gives error "Error in eval(predvars, data, env) : object 'case' not found". This seems to work? I do not understand why and it just creates a column of zeros called "case".
...
#Fitting glmm, 
pop.rsf_winter_21 <- glmer(case ~ AG + HW +  NP + H_P + PP + P_H + S_S + U_M + WL + WFP + (1|trap_locat),
                 data = rsfData_modified_winter_21[-which(rsfData_modified_winter_21$trap_locat == 115),],
                 na.action = "na.pass",
                 family = "binomial")
...
pop.dre <- dredge(pop.rsf_winter_21, fixed = ~(1|trap_locat))
