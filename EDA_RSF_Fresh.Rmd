...
habitat <- raster::shapefile("/Volumes/Samsung_T5/BOBWHITE_DATA/Maps/2022_LCov_BurnStatus/SSburned/LCOVBurnedUnburnedMergedSSburned.shp")
...
Looking at all birds as a mixed effects model.
```{r}
...
coefficients_data_no_intercept %>%
  ...
  xlab('Variable') +
  ylab('Beta Coefficient') +
  ggtitle('Beta coefficients for all males') +
  theme_minimal()
...
coefficients_data_no_intercept %>%
  ...
  xlab('Variable') +
  ylab('Beta Coefficient') +
  ggtitle('Beta coefficients for all females') +
  theme_minimal()
...
habitat$LCOV_MGMT <- gsub("AG", "unburned", habitat$LCOV_MGMT)
habitat$LCOV_MGMT <- gsub("WL", "unburned", habitat$LCOV_MGMT)
habitat$LCOV_MGMT <- gsub("UM", "unburned", habitat$LCOV_MGMT)
habitat$LCOV_MGMT <- gsub("NP_UB", "unburned", habitat$LCOV_MGMT)
habitat$LCOV_MGMT <- gsub("PP_UB", "unburned", habitat$LCOV_MGMT)
habitat$LCOV_MGMT <- gsub("HW_UB", "unburned", habitat$LCOV_MGMT)
habitat$LCOV_MGMT <- gsub("SS_UB", "unburned", habitat$LCOV_MGMT)
habitat$LCOV_MGMT <- gsub("NP_BU", "burned", habitat$LCOV_MGMT)
habitat$LCOV_MGMT <- gsub("PP_BU", "burned", habitat$LCOV_MGMT)
habitat$LCOV_MGMT <- gsub("HW_BU", "burned", habitat$LCOV_MGMT)
habitat$LCOV_MGMT <- gsub("SS_BU", "burned", habitat$LCOV_MGMT)
...
rsfData_summer2022_burned_unburned <- Summer_2022_locs_all_age_sex %>% 
  #Converting it to a tibble
  as_tibble() %>%
   #Removing timezone offset, letter "T", creating DT.GMT column by converting to POSIX format, creating DT column by subtracting 5 hrs (time zones)
  mutate(DT.chr = gsub("-05:00","",gsub("T"," ",date_created)),
...
#Creating column "case" where TRUEs (known points) have value of 1, and Falses (random points) have value of 0. Standardizing our covariates around the center.
rsfData_modified_summer2022_burned_unburned <- rsfData_summer2022 %>% 
  mutate(case = ifelse(case_ == T, 1, 0)) %>%
...
Summer_2023_locs <- rio::import("/Volumes/Samsung_T5/BOBWHITE_DATA/Clean/Summer2023Locs_Clean3.xlsx", setclass = "tibble") %>%
  clean_names() %>%
  filter(status != "D",
         status != "N") %>%
  mutate(date = lubridate::as_date(date))
...
#Importing "fate" spreadsheet, which has sex and age-class associated with each band number (unlike daily locations data)
fate <- rio::import("/Volumes/Samsung_T5/BOBWHITE_DATA/Clean/fate_all_birds_clean4880EDIT.xlsx", setclass = "tibble") %>%
  clean_names() 
...
#Isolating Juve Males
Summer_2023_locs_males_juv <- Summer_2023_locs_all_age_sex %>%
  filter(sex == "M",
         age == "J")

#Creating tibble for RSF, includes random and known points for each NOBO within kde home range, as well as extracted covariates.  
rsfData_summer2023_males_juv <- Summer_2023_locs_males_juv %>% 
  #Converting it to a tibble
  as_tibble() %>%
   #Removing timezone offset, letter "T", creating DT.GMT column by converting to POSIX format, creating DT column by subtracting 5 hrs (time zones)
  mutate(DT.chr = gsub("-05:00","",gsub("T"," ",date_created)),
...
#Creating column "case" where TRUEs (known points) have value of 1, and Falses (random points) have value of 0. Standardizing our covariates around the center.
rsfData_modified_summer2023_males_juv <- rsfData_summer2023_males_juv %>% 
  mutate(case = ifelse(case_ == T, 1, 0)) %>%
  mutate_at(vars(AG:WL), .funs = function(x){as.numeric(scale(x, center = T))}) 

#Isolating Adult males
Summer_2023_locs_males_adult <- Summer_2023_locs_all_age_sex %>%
  filter(sex == "M",
         age == "A")

#Creating tibble for RSF, includes random and known points for each NOBO within kde home range, as well as extracted covariates.  
rsfData_summer2023_males_adult <- Summer_2023_locs_males_adult %>% 
  #Converting it to a tibble
  as_tibble() %>%
   #Removing timezone offset, letter "T", creating DT.GMT column by converting to POSIX format, creating DT column by subtracting 5 hrs (time zones)
  mutate(DT.chr = gsub("-05:00","",gsub("T"," ",date_created)),
...
#Creating column "case" where TRUEs (known points) have value of 1, and Falses (random points) have value of 0. Standardizing our covariates around the center.
rsfData_modified_summer2023_males_adult <- rsfData_summer2023_males_adult %>% 
  mutate(case = ifelse(case_ == T, 1, 0)) %>%
  mutate_at(vars(AG:WL), .funs = function(x){as.numeric(scale(x, center = T))}) 
...
#Isolating Juv females
Summer_2023_locs_females_juv <- Summer_2023_locs_all_age_sex %>%
  filter(sex == "F",
         age == "J")

#Creating tibble for RSF, includes random and known points for each NOBO within kde home range, as well as extracted covariates.  
rsfData_summer2023_females_juv <- Summer_2023_locs_females_juv %>% 
  #Converting it to a tibble
  as_tibble() %>%
   #Removing timezone offset, letter "T", creating DT.GMT column by converting to POSIX format, creating DT column by subtracting 5 hrs (time zones)
  mutate(DT.chr = gsub("-05:00","",gsub("T"," ",date_created)),
...
#Creating column "case" where TRUEs (known points) have value of 1, and Falses (random points) have value of 0. Standardizing our covariates around the center.
rsfData_modified_summer2023_females_juv <- rsfData_summer2023_females_juv %>% 
  mutate(case = ifelse(case_ == T, 1, 0)) %>%
  mutate_at(vars(AG:WL), .funs = function(x){as.numeric(scale(x, center = T))}) 

#Isolating Adult Females
Summer_2023_locs_females_adult <- Summer_2023_locs_all_age_sex %>%
  filter(sex == "F",
         age == "A")

#Creating tibble for RSF, includes random and known points for each NOBO within kde home range, as well as extracted covariates.  
rsfData_summer2023_females_adult <- Summer_2023_locs_females_adult %>% 
  #Converting it to a tibble
  as_tibble() %>%
   #Removing timezone offset, letter "T", creating DT.GMT column by converting to POSIX format, creating DT column by subtracting 5 hrs (time zones)
  mutate(DT.chr = gsub("-05:00","",gsub("T"," ",date_created)),
...
#Creating column "case" where TRUEs (known points) have value of 1, and Falses (random points) have value of 0. Standardizing our covariates around the center.
rsfData_modified_summer2023_females_adult <- rsfData_summer2023_females_adult %>% 
  mutate(case = ifelse(case_ == T, 1, 0)) %>%
  mutate_at(vars(AG:WL), .funs = function(x){as.numeric(scale(x, center = T))}) 

```

Summer 2023. Looking at all ages and sexes 
```{r}
#### Summer 2023 all ages and sexes Bayesian #### 
pop.rsf_summer2023_bayes <- brm(
...
#Printing summary of results.
summary(pop.rsf_summer2023_bayes)

# Get the summary of the model
model_summary_summer2023_bayes <- summary(pop.rsf_summer2023_bayes)
...
coefficients_data_no_intercept %>%
  ...
  xlab('Variable') +
  ylab('Beta Coefficient') +
  ggtitle('Summer 2023: Beta coefficients for all age and sex classes') +
  theme_minimal()
