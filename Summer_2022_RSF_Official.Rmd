...
#Data set with 1 loc per week, 1 per day.
...
rsfData_modified_summer2022_early_late <- rsfData_summer2022 %>% 
...
#Fitting glm
pop.rsf_summer2022_all_birds <- glmer(case ~ AG + UM + NP_UB +NP_BU + PP_UB + WL + PP_BU + HW_UB + HW_BU + SS_UB + SS_BU + (1|band_numb),
...
```{r}
#All females
pop.rsf_summer2022_females <- glmer(case ~ AG + UM + WL + NP_UB +NP_BU + PP_UB + PP_BU + HW + HP_UB + HP_BU + SS_UB + SS_BU + (1|band_numb),
...
pop.rsf_summer2022_bayes_TS <- brm(
  ...
  case | trials(1) ~ AG + UM + NP_UB +NP_BU + PP_UB + PP_BU + HW + HP_UB + HP_BU + SS_UB + SS_BU + WL + (1|band_numb),
...
summary(pop.rsf_summer2022_bayes_TS)

# Get the summary of the model
model_summary_summer2022_bayes_TS <- summary(pop.rsf_summer2022_bayes_TS)


#Below we calculate our logs odds ratios.
# Extract the fixed effects (estimates) from the model summary
fixed_effects_TS <- model_summary_summer2022_bayes_TS$fixed

# Create a data frame from the fixed effects
coefficients_data <- as.data.frame(fixed_effects_TS)
...
coefficients_data_no_intercept_TS <- coefficients_data %>%
  filter(Variable != "Intercept")

coefficients_data_no_intercept_TS$TS <- "YES"

#coefficients_data_no_intercept_NO_TS <- coefficients_data %>%
  #filter(Variable != "Intercept")

coCombined <- rbind(coefficients_data_no_intercept_NO_TS, coefficients_data_no_intercept_TS)
...
coCombined %>%
  ...
  ggtitle('2022: Beta coefficients: Including and Excluding data from Telem staions') +
  theme_minimal()

coefficients_2022_10rand <- coefficients_data_no_intercept

writexl::write_xlsx(coefficients_5_rand_points_2022, "/Users/jeffgrayum/Downloads/2022_coefficients.xlsx")
...
habitat$LCOV_MGMT2 <- gsub("AG", "unburned", habitat$LCOV_MGMT2)
habitat$LCOV_MGMT2 <- gsub("WL", "unburned", habitat$LCOV_MGMT2)
habitat$LCOV_MGMT2 <- gsub("UM", "unburned", habitat$LCOV_MGMT2)
habitat$LCOV_MGMT2 <- gsub("NP_UB", "unburned", habitat$LCOV_MGMT2)
habitat$LCOV_MGMT2 <- gsub("PP_UB", "unburned", habitat$LCOV_MGMT2)
habitat$LCOV_MGMT2 <- gsub("HW_UB", "unburned", habitat$LCOV_MGMT2)
...
habitat$LCOV_MGMT2 <- gsub("HW_BU", "burned", habitat$LCOV_MGMT2)
