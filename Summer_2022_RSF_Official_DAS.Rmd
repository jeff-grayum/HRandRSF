...
habitat <- raster::shapefile("/Volumes/Samsung_T5/ThesisStudySite/LCOV_UPDATE/2022_FL_RC/_2022_WFP_FL_Final.shp")
...
#Importing map
#amount of each land cover type in 2022
habitat22 <- raster::shapefile("/Volumes/Samsung_T5/ThesisStudySite/LCOV_UPDATE/2022/UPDATED_2022_LCOV_MGMT.shp")


# Converting summer_2022_locs to SpatialPointsDataFrame
coordinates(Summer_2022_locs) <- ~easting + northing 

#Matching coord system with habitat layer
proj4string(Summer_2022_locs) <- proj4string(habitat22)

# Overlay locations onto the habitat map
bobwhite_lcov22 <- over(Summer_2022_locs, habitat22)

# Add lcov type to Summer 2022 locs
Summer_2022_locs$LCOV_MGMT <- bobwhite_lcov22$LCOV_MGMT

# Now summer_2022_locs contains the landcover type each bobwhite was in

# Plotting locations on the habitat map to make sure stuff makes sense
plot(habitat)
points(Summer_2022_locs, col = 'red', pch = 20) # change color and point character as needed
...
OVER_25_LOCS22 <- Summer_2022_locs %>%
  count(band_numb, sort = TRUE) %>%
  dplyr::filter(n >= 25) %>%
  dplyr::pull(band_numb)

Summer_2022_locs25 <- Summer_2022_locs %>%
  dplyr::filter(band_numb %in% OVER_25_LOCS22) 

#Making list of lcov type of birds, only using birds w 25 locs
Summer_2022_locs25LCOV <- Summer_2022_locs %>%
  select(LCOV_MGMT)

#Making tibble of number of lcov 
LCOV_SUM22 <- Summer_2022_locs25LCOV %>%
...
#Let's count the land cover types.
lcov_sums22 <- habitat22 %>%
  as.data.frame() %>%
  group_by(LCOV_MGMT) %>%
  summarize(sum_acres22 = sum(ACRES))

totalacres22 <- habitat22 %>%
  as.tibble () 

totalacres22 <- sum(totalacres22$ACRES)

```


Creating tibbles for RSF
```{r}
#### Creating Summer 2022 tibbles for RSFs, all birds ####
...
#Creating a non centered/scaled version so we can backtransform the x-axis in our predict plots.
rsfData_summer22_UCS <- rsfData_summer2022 %>% 
  mutate(case = ifelse(case_ == T, 1, 0))


#### Creating summer 2022 tibble for females ####
...
#### Creating summer 2022 tibble for males ####
...
habitat <- raster::shapefile("/Volumes/Samsung_T5/ThesisStudySite/LCOV_UPDATE/2022/UPDATED_2022_LCOV_MGMT.shp")
...
coefficients_data_no_intercept %>%
  ...
  xlab('Variable') +
  ylab('Beta Coefficient') +
  ggtitle('Beta coefficients: burned and unburned areas 2022') +
  theme_minimal()
...
```{r}
#### Birds trapped each season ####

fate <- readxl::read_xlsx("/Volumes/Samsung_T5/BOBWHITE_DATA/Clean/Fate/fate_all_birds_clean.xlsx") %>%
  clean_names()

fate %>% 
  view()

fate %>% count(course_season)
```

Lets attempt to run a RSF on basal area, only
```{r}
#### Basal area RSF ####
#Importing map of study site, setting template for raster stack.
...
#Viewing habitat shapefile. Land cover types here are now numeric values.
plot(habitat, col = as.factor(habitat$LCDENSITYS))
```
...
```{r}
# Load the shapefile
habitat_basal <- raster::shapefile("/Volumes/Samsung_T5/ThesisStudySite/LCOV_UPDATE/2022/UPDATED_2022_LCOV_MGMT.shp")

habitat_basal_df <- as.data.frame(habitat_basal)
#View(habitat_basal_df)

# Assuming your data frame is called 'data_frame'
habitat_basal_df<- habitat_basal_df %>%
mutate(LCDENSITYS = as.character(LCDENSITYS)) %>%  # Convert factors to characters if necessary
  mutate(LCDENSITYS = case_when(
    LCDENSITYS == "NA" ~ "0",  # Replace "NA" with character "0" to avoid confusion with actual NA
    LCDENSITYS == "<30" ~ "15",
    LCDENSITYS == "30-60" ~ "45",
    LCDENSITYS == "60-90" ~ "75",
    LCDENSITYS == ">90" ~ "100",
    TRUE ~ LCDENSITYS  # Keep other values as they are
  )) %>%
...
habitat_basalNP <- habitat_basal_df %>%
  dplyr::filter(LCOV %in% "NP") %>%
  summarize(median_ba = median(LCDENSITYS))
...
Creating our stack of raster layers. This will be used in Summer 2022 RSF for basal area.
```{r}
...
#Now we stack our raster layers.
distanceStack22_basal <- stack(rasterList)

plot(distanceStack22_basal)
```
...
```{r}
rsfData_summer2022_basal <- Summer_2022_locs %>% 
  #Converting it to a tibble
   as_tibble() %>%
   #Removing timezone offset, letter "T", creating DT.GMT column by converting to POSIX format, creating DT column by subtracting 5 hrs (time zones)
  mutate(DT.chr = gsub("-05:00","",gsub("T"," ",date_created)),
...
#Creating column "case" where TRUEs (known points) have value of 1, and Falses (random points) have value of 0. Standardizing our covariates around the center.
rsfData_modified_summer2022_basal <- rsfData_summer2022_basal  %>% 
  mutate(case = ifelse(case_ == T, 1, 0)) %>%
