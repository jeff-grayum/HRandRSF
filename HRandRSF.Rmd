...
small_hr <- wint_avail_and_selection %>%
...
med_hr <- wint_mcp_percent_lcov_clean %>%
  filter(hr_size > 6.724,
         hr_size < 13.326) %>%
  view()
...
med_hr %>%
  ggplot(aes(lcov, percent_hab/100, fill = lcov)) +
  geom_col() +
  facet_wrap(~ band_numb) +
  theme_minimal() +
  labs(x = "Land cover",
       y = "Availability",
       title = "Land cover availability for NOBO with medium-sized home ranges") +
  scale_y_continuous(labels = percent) 

#Looking at availability for large home ranges
large_hr <- wint_mcp_percent_lcov_clean %>%
...
large_hr %>%
  ...
  labs(x = "Land cover",
       y = "Availability",
       title = "Land cover availability for NOBO with large home ranges") +
  scale_y_continuous(labels = percent) 
```
