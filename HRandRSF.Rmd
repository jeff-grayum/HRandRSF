...
Estimating MCP home ranges for bobwhite with 5+ observations (Min required sample size)
```{r}
...
write.csv(SS_5_mcp_hr_size_winter_2021_22,"/Users/jeffgrayum/Downloads/MCP_covey_HR_2021_22.csv", row.names = FALSE)
...
plot(SF_StudySite, alpha = 0.5, add = TRUE)
plot(SS_5_winter_2021_22_locs_SPDF, col = as.data.frame(SS_5_winter_2021_22_locs_SPDF)[,10], add = TRUE)
...
write.csv(covey_kde_hr_across_levels_2021_22,"/Users/jeffgrayum/Downloads/Covey_KDE_HR_By_BandNumb_2021_22.csv", row.names = TRUE)
...
mcp_vs_kde_pivot <- mcp_vs_kde %>%
  rename(KDE = kde_area,
         MCP = mcp_area) %>%
  pivot_longer(MCP:KDE, names_to = "analysis", values_to = "ha")

mcp_vs_kde_pivot %>%
  ggplot(aes(analysis, ha, fill = analysis)) +
  geom_boxplot() +
  labs(x = "Method",
       y = "Home range size (ha)",
