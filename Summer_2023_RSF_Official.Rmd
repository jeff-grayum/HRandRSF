...
Summer_2023_locs <- rio::import("/Volumes/Samsung_T5/BOBWHITE_DATA/Clean/SummerLocations/Summer2023/Summer2023Locs_Clean3.xlsx", setclass = "tibble") %>%
  clean_names() %>%
  filter(status == "A") %>%
...
rsfData_summer2023_males <- Summer_2023_locs_males %>% 
  ...
  mutate(indData = map2(tracks, kde, function(x,y){
      ...
      #Below we generate x random points within each NOBO's KDE home range.
      random_points(y, n = nrow(x) * 5, level=.95, presence = x) %>%
      #Extracting covariates (lcov types) of known and random points
      extract_covariates(distanceStack)
  })) %>%
  #Unnesting data back into original rows.
...
coefficients_data_no_intercept_2023 <- coefficients_data_no_intercept

coefficients_data_no_intercept_2023$Year <- "2023"

cd22 <- read_xlsx("/Users/jeffgrayum/Downloads/2022_coefficients.xlsx")

both_years <- rbind(coefficients_data_no_intercept_2023, cd22)

# Filter out the intercept and plot
both_years %>%
  ggplot(aes(x= Variable, y = Estimate, color = Year)) +
  ...
  ggtitle('Summer 2022 and 2023 Beta coefficients') +
  theme_minimal()
