...
#AG TIME
#Let's start with NP_BU
ag_bu_values <- seq(min(rsf_modified_2022_23$AG, na.rm = TRUE), 
                 max(rsf_modified_2022_23$AG, na.rm = TRUE), 
                 length.out = 1000)

ag_bu_df <- data.frame(
  NP_BU = rep(mean(rsf_modified_2022_23$NP_BU), length(np_bu_values)),
  AG = ag_bu_values,
  HP_BU = rep(mean(rsf_modified_2022_23$HP_BU), length(np_bu_values)),
  HP_UB = rep(mean(rsf_modified_2022_23$HP_UB), length(np_bu_values)),
  HW = rep(mean(rsf_modified_2022_23$HW), length(np_bu_values)),
...
ag_bu_pred_draws <- epred_draws(newdata = ag_bu_df, object = pred_model, value = ".prediction", seed = 123, ndraws = 10000)

ag_bu_pred_draws_sum <- ag_bu_pred_draws%>%
  group_by(AG) %>%
  summarize(mean_fit = mean(.prediction),
            sd_fit = sd(.prediction))
