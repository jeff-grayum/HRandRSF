...
nobo_count <- rsf_modified_2022_23 %>%
  filter(case_ == TRUE) %>%
...
#saveRDS(pop.rsf_summer2022_23_bayes, "/Volumes/Samsung_T5/BOBWHITE_DATA/Clean/Other/model_RSFsummer2022_23.rds")
```
...
pop.rsf_summer2022_23_bayes <- readRDS("/Volumes/Samsung_T5/BOBWHITE_DATA/Clean/Other/model_RSFsummer2022_23.rds")
...
#writexl::write_xlsx(coefficients_data_no_intercept_2022_2023, "/Volumes/Samsung_T5/ThesisPlots/Ch1/betasLCOV22_23.xlsx")
...
# Get the posterior samples as a data frame (posterior package deprecated)...
posterior_draws_bu <- as_draws_df(pop.rsf_summer2022_23_bayes)
...
posterior_draws_long_bu %>%
  ...
  facet_wrap(~factor(landcover, levels = c("NP_BU", "AG","HP_UB", "SS_UB","PP_BU", "SS_BU", "HW", "UM", "NP_UB", "HP_BU")), ncol = 5) +
  theme_minimal() +
  labs(x = "Distribution of estimated posterior beta coefficient",
      y = "Density") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
   theme_bw() +
  theme(legend.position = "none") +
...
writexl::write_xlsx(coefficients_data_no_intercept_allBU_UB, "/Users/jeffgrayum/Downloads/all_bu_ub.xlsx")
...
np_bu_pred_draws <- epred_draws(newdata = np_bu_df, object = pred_model, value = ".prediction", seed = 123, ndraws = 10000)
...
ag_pred_draws <- epred_draws(newdata = ag_df, object = pred_model, value = ".prediction", seed = 123, ndraws = 10000)
...
np_ub_pred_draws <- epred_draws(newdata = np_ub_df, object = pred_model, value = ".prediction", seed = 123, ndraws = 10000)
...
hp_bu_pred_draws <- epred_draws(newdata = hp_bu_df, object = pred_model, value = ".prediction", seed = 123, ndraws = 10000)
...
all_covs_pred <- rbind(np_bu_pred_draws_sum_piv, ag_pred_draws_sum_piv, np_ub_pred_draws_sum_piv, hp_bu_pred_draws_sum_piv)

library(ggplot2)
library(dplyr)

# Re-ordering the legend for a non-facet_wrapped plot
all_covs_pred$Covariate <- fct_relevel(all_covs_pred$Covariate, "HP_BU", "NP_UB", "AG", "NP_BU")
...
all_covs_pred %>%
  ...
  scale_color_manual(values = c("HP_BU" = "brown", "NP_UB" = "blue", "AG" = "#F0E68C", "NP_BU" = "#006400")) +
  scale_fill_manual(values = c("HP_BU" = "brown", "NP_UB" = "blue", "AG" = "#F0E68C", "NP_BU" = "#006400")) +
  labs(x = "Distance to covariate (centered and scaled)", 
       y = "Probability of selection") +
  theme_bw()

# Re-ordering the legend for a facet_wrapped plot
all_covs_pred$Covariate <- fct_relevel(all_covs_pred$Covariate, "NP_BU", "AG", "NP_UB", "HP_BU")

#Making a non-facet-wrapped plot
all_covs_pred %>%
  ggplot(aes(value, mean_fit)) +
  geom_line(aes(color = Covariate)) +
  geom_ribbon(aes(ymin = low_ci, ymax = upper_ci, fill = Covariate), alpha = 0.5) +
  scale_color_manual(values = c("HP_BU" = "brown", "NP_UB" = "blue", "AG" = "#F0E68C", "NP_BU" = "#006400")) +
  scale_fill_manual(values = c("HP_BU" = "brown", "NP_UB" = "blue", "AG" = "#F0E68C", "NP_BU" = "#006400")) +
  labs(x = "Distance to covariate (centered and scaled)", 
       y = "Probability of selection") +
...
pred_plot_NP_BU_JM <- ggplot(np_bu_pred_draws_sum,
  ...
  scale_x_continuous(breaks=seq(min(np_bu_pred_draws_sum$NP_BU),max(np_bu_pred_draws_sum$NP_BU),length.out = 10),
                     labels=(round((seq(min(np_bu_pred_draws_sum$NP_BU),max(np_bu_pred_draws_sum$NP_BU),length.out = 10)*sd(binded_22_23_uncs$NP_BU)*2.54+mean(binded_22_23_uncs$NP_BU)*2.54),1)))
